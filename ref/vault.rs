...
fn parse_hex_32(s: &str) -> StdResult<[u8; 32]> {
    let h = if s.starts_with("0x") || s.starts_with("0X") { &s[2..] } else { s };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetVerifier { verifier_pubkey } => exec_set_verifier(deps, info, verifier_pubkey),
...
fn exec_emergency_withdraw(
    ...
    let msg: CosmosMsg = match asset {
        AssetInfo::Native { denom } => {
            let coin = Coin { denom, amount: value };
            BankMsg::Send {
